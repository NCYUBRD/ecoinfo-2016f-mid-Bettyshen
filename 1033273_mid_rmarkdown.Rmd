...
rownames(Month_collumn_trans)<-a$name
View(Month_collumn_trans)


```{r}
#計算2014和2015年最冷月分別在哪個月份？
#做出最冷min的方程式
coldest<-function(x){
  min(x, na.rm = TRUE)
}
...
#2015最冷月份"2"月，該月中每日最低溫平均是幾度C?
```{r}
#將'月'與'日'結合起來(group_by)，先算每日最低溫
Month_day<- cbind(sample_data_allcol,month,day)
